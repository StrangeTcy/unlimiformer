...
def report_cuda_mem():
    torch.cuda.init()
    for i in trange(torch.cuda.device_count()):
        device = torch.device(f"cuda:{i}")
        a = torch.Tensor([0])
        a = a.to(device)
        print(f"for device {device}")
        print(f"torch.cuda.memory_allocated: {bytes2human(torch.cuda.memory_allocated(i))} GB")  #/(1024**3)
        print(f"torch.cuda.memory_reserved: {bytes2human(torch.cuda.memory_reserved(i))} GB")  #/(1024**3)
        print(f"torch.cuda.max_memory_reserved: {bytes2human(torch.cuda.max_memory_reserved(i))} GB")  #/(1024**3)



def memory_stats(model):
    report_cuda_mem()
    print(f"our model's memory footprint is {bytes2human(model.get_memory_footprint())}")
...
def main():
    ...
    print("actually parsing our args...")
    args, unknown_args = parser.parse_known_args()

    print("parsing some more args with a HfArgumentParser...")
    ...
    print("setting seed for our RNGs...")
    set_seed(args)

    # Initialize the model and tokenizer
    try:
        args.model_type = args.model_type.lower()
        print("instantiating our model...")
    ...
    print("and now instantiating our tokenizer...")
    ...
    print(f"placing our model on the device selected above (it's {args.device})...")
    ...
    print(f"adjusting length to model, at the moment it's {args.length}...")
    args.length = adjust_length_to_model(args.length, max_sequence_length=max_seq_length)
    logger.info(args)
    print(f"we now have length {args.length}")
    ...
    print("examining the results...")

    for generated_sequence_idx, generated_sequence in enumerate(tqdm(output_sequences)):
